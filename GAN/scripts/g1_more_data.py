...
# # GAN — g1 more data
# 
# Esta versão parte da baseline instrumentada **g0**, mas altera a quantidade de dados reais usados no treino.
# 
# O objetivo da fase **g1** é testar se a limitação principal da DCGAN baseline era o tamanho reduzido do dataset. Para isso, mantemos a mesma arquitetura, os mesmos hiperparâmetros e a mesma instrumentação do g0, mas aumentamos o intervalo de folds reais usados.
# 
# Mantém-se:
# 
# - outputs separados por `RUN_NAME`;
# - histórico de treino em CSV;
# - samples fixos em PNG;
# - FID/KID opcional quando `torchmetrics` estiver disponível;
# - interpolação latente no fim;
# - checkpoints selecionados;
# - mesma arquitetura DCGAN;
# - mesmos learning rates;
# - sem EMA;
# - sem SpectralNorm;
# - sem label smoothing;
# - sem alterações no discriminador.
# 
# Alteração principal desta fase:
# 
# - aumento dos folds reais usados no treino, por exemplo de `START_FOLD = 0, END_FOLD = 5` para `START_FOLD = 0, END_FOLD = 20`.
# 
# Hipótese:
# 
# > Com mais imagens reais, o gerador deverá ter acesso a maior diversidade visual, reduzindo overfitting e melhorando a qualidade/diversidade das imagens geradas.
# 
# Critérios de comparação com g0:
# 
# - FID final;
# - melhor FID;
# - epoch do melhor FID;
# - curvas `D(x)` e `D(G(z))`;
# - qualidade visual das samples fixas;
# - interpolação latente final.
...
# =========================================================
# Config — g0 baseline instrumented
# =========================================================
# Esta experiência é a baseline mensurável: mantém arquitetura e hiperparâmetros base.

RUN_NAME = "g1_more_data"
...
# Folds usados na baseline g0. Não alterar nesta fase.
START_FOLD = 0
END_FOLD = 20
INTERVAL = True
...
CHECKPOINT_EVERY = 5      # guardar checkpoint a cada N epochs
...
def save_samples_jsonl(epoch, generator, noise, path, nrow=8):
    ...
    Útil se quiseres guardar uma evolução por epoch num formato único.
    Nesta versão g0 está desligado por defeito através de SAVE_JSONL_EVERY=None.
    """
    generator.eval()
    with torch.no_grad():
...
# =========================================================
# Treino — g0 baseline instrumented
# =========================================================
# Mantém o treino adversarial da baseline, mas guarda métricas e artefactos de forma organizada.
...
for epoch in epoch_bar:
    ...
    # Samples PNG selecionadas
    if should_run(epoch, SAMPLE_EVERY):
        save_generated_samples_png(epoch, netG, fixed_noise, prefix="fixed_noise")

    # JSONL opcional
    if SAVE_JSONL_EVERY is not None and should_run(epoch, SAVE_JSONL_EVERY):
        save_samples_jsonl(epoch, netG, fixed_noise, samples_jsonl_path)

    # FID/KID selecionado
    if FID_EVERY is not None and (epoch % FID_EVERY == 0 or epoch == NUM_EPOCHS):
        metrics = compute_fid_kid(netG, dataloader, num_images=FID_NUM_IMAGES, compute_kid=COMPUTE_KID)
        metrics.update({"run_name": RUN_NAME, "epoch": epoch})
        fid_history.append(metrics)
        pd.DataFrame(fid_history).to_csv(fid_path, index=False)
        if COMPUTE_KID:
            print(f"Epoch {epoch:03d} | FID: {metrics['fid']:.3f} | KID: {metrics['kid_mean']:.5f} ± {metrics['kid_std']:.5f}")
        else:
            print(f"Epoch {epoch:03d} | FID: {metrics['fid']:.3f} | KID: desligado")
    ...
    if should_run(epoch, CHECKPOINT_EVERY):
        torch.save({
            ...
            "config": run_config,
        }, os.path.join(CHECKPOINTS_DIR, f"dcgan_{RUN_NAME}_epoch_{epoch:03d}.pt"))
...
if os.path.exists(fid_path):
    ...
    if "kid_mean" in fid_df.columns and fid_df["kid_mean"].notna().any():
        ...
        print("KID desligado ou sem valores válidos; a avaliação g0 usa FID como métrica principal.")
...
with torch.no_grad():
    ...
    fake_images = denorm_to_01(netG(noise).cpu())
...
summary = {
    "Iteration": "g0_baseline_instrumented",
    "Change": "DCGAN baseline + FID/KID logging + fixed samples + latent interpolation",
    "FID @ end": None,
    "Notes": "Baseline mensurável; sem EMA, sem SpectralNorm, sem alterações de LR/dados.",
}
...
# **FID/KID:**
# - Aumentar os folds de 0–5 para 0–20 melhorou bastante o gerador.
# 
# | Epoch | FID | KID mean |
# |------:|----:|---------:|
# | 50    | 260.38 | 0.1389 |
...
# | Métrica      |            g0 |                        g1 |
# | ------------ | ------------: | ------------------------: |
# | Best FID     |        ~300.4 |                **208.99** |
# | FID final    |        ~307.2 |                **208.99** |
# | Melhor epoch |           150 |                   **200** |
# | Visual       | muito ruidoso | **mais estrutura humana** |
# 
# - Isto é uma melhoria de cerca de 91 pontos de FID face ao melhor g0, e quase 98 pontos face ao FID final do g0.
# 
# **Discriminador:**
# - O g1 confirma a hipótese:
# 
#       O dataset limitado era uma das principais causas da fraca qualidade da baseline.
# 
# - Com mais imagens reais, o gerador aprendeu melhor: silhuetas humanas, cabeças/rostos aproximados, contraste de retratos, roupa/fundo, variação de pose/cor, etc
# - As imagens continuam imperfeitas, mas são claramente menos abstratas do que no g0.
# 
# **Atenção: o discriminador continua dominante:**
# - Apesar da melhoria, o gráfico mostra que o problema adversarial continua:
# 
#       D(x) final ≈ 0.991
#       D(G(z)) fake final ≈ 0.0088
#       D(G(z)) gen final ≈ 0.0050
#       Loss_G final ≈ 6.14
#       Loss_D final ≈ 0.018
# 
# - Isto quer dizer que, no fim, o discriminador ainda distingue falsos de reais com quase total confiança.
# - Ou seja: g1 melhorou por causa de mais dados, mas não resolveu o desequilíbrio G/D.
# 
# 
# **Nota sobre os spikes:**
# - Os gráficos mostram alguns picos periódicos em `loss_d`, `D(G(z))` fake e `D(G(z))` gen. Isso não é necessariamente mau; parecem momentos em que o gerador encontra temporariamente uma direção melhor, mas o discriminador recupera rapidamen

# ---
# 
# In g1, we increased the number of real-image folds while keeping the DCGAN architecture and training hyperparameters unchanged. This isolated the effect of dataset scale. The FID improved from approximately 300.4 in g0 to 209.0 at epoch 200, and the generated samples showed clearer human-like structure. However, discriminator outputs still saturated, with D(x) close to 1 and D(G(z)) close to 0, indicating that the adversarial game remained unbalanced.

# | g1 | more_data | Increase real training folds from 0–5 to 0–20, keeping architecture and hyperparameters fixed | FID improved from g0 best ≈300.4 to 208.99 @ epoch 200; KID=0.0887 | More data substantially improves sample quality and distribution matching. However, D still dominates: D(x)=0.991 and D(G(z))≈0.005 at epoch 200. Proceed to discriminator balancing. |
