...
# # GAN — g0 baseline instrumented
# 
# Esta versão mantém a DCGAN base, mas acrescenta a instrumentação necessária para a fase **g0**:
# 
# - outputs separados por `RUN_NAME`;
# - histórico de treino em CSV;
# - samples fixos em PNG;
# - FID/KID quando `torchmetrics` estiver disponível;
# - interpolação latente no fim;
# - checkpoints selecionados em vez de checkpoint por epoch.
# 
# **Importante:** nesta fase não há ainda EMA, SpectralNorm, mais folds, label smoothing ou alterações de learning rate. O objetivo é criar uma baseline mensurável.
# 
...
RUN_NAME = "g0_baseline_instrumented_with_KID"
...
# **FID:**
# - A baseline DCGAN reduziu o FID entre as epochs 50 e 150, mas a melhoria estabilizou e degradou ligeiramente até à epoch 200, sugerindo limitação da arquitetura/dados e possível saturação adversarial.
# 
# **Discriminador:**
# 
#      D(x) → sobe até ~0.98
#      D(G(z)) fake → desce para ~0.01–0.02
#      D(G(z)) gen → também fica muito baixo
# 
# - Observámos que D(x) se aproxima de 1 enquanto D(G(z)) tende para valores próximos de 0. Isto indica que o discriminador se tornou demasiado forte, reduzindo o sinal útil para o gerador.
# 
# **Imagens geradas na epoch 200:**
# - As imagens mostram que a DCGAN já aprendeu algumas propriedades globais: cores de pele, fundos, silhuetas, algumas regiões tipo rosto/cabeça, contraste geral de retratos. 
# - Mas ainda há muitos problemas: rostos pouco, muito ruído, artefactos cromáticos fortes, baixa coerência facial, algumas imagens quase abstratas, pouca nitidez
# - Ou seja, visualmente está coerente com um FID alto.
# - A baseline aprende a distribuição geral de cores e composição de retratos, mas não consegue gerar faces realistas nem preservar estrutura facial consistente.
# 
# **Interpolação Latente:**
# - A sequência parece quase uma transição entre manchas/ruído, não entre identidades ou faces claras. Isto sugere que:
#      - o espaço latente ainda não está bem estruturado
#      - o gerador não aprendeu uma manifold facial robusta
#      - há baixa qualidade e possível baixa diversidade útil
# - A interpolação latente da baseline mostra transições suaves em cor/textura, mas não em atributos faciais semânticos, indicando que o espaço latente ainda não codifica bem características humanas.

# ---
# 
# g0 — Baseline DCGAN instrumentada
# 
# **Alteração:**
# Baseline DCGAN com logging completo, FID/KID, samples fixos e interpolação latente.
# 
# **Resultado:**
# FID melhorou de ~328.8 na epoch 50 para ~300.4 na epoch 150, mas piorou para ~307.2 na epoch 200.
# As imagens geradas mostram estrutura global de retratos, mas faces pouco coerentes e muito ruído.
# O discriminador domina o treino: D(x) aproxima-se de 1 e D(G(z)) aproxima-se de 0.
# 
# **Decisão:**
# Usar g0 como baseline. Próxima fase deve atacar limitação de dados e desequilíbrio adversarial.

# | g0 | baseline_instrumented | DCGAN baseline + FID/KID logging + fixed samples + latent interpolation | Best FID ≈ 300.4 @ epoch 150; FID @ epoch 200 ≈ 307.2 | Baseline measurable. Generator learns rough portrait colors/layout but faces remain noisy. D dominates: D(x)→~0.98 and D(G(z))→~0.01. Proceed to more data and discriminator balancing. |
