...
# # GAN — g2 discriminator balancing
# 
# Esta versão parte do g1_more_data e mantém a mesma arquitetura e os mesmos folds reais.
# 
# A única alteração experimental é o balanceamento do jogo adversarial:
# 
# - redução do learning rate do discriminador;
# - label smoothing nos alvos reais.
# 
# Hipótese:
# 
# > Como no g1 o discriminador continuava a saturar, reduzimos a sua velocidade de aprendizagem e suavizámos os labels reais para evitar confiança excessiva. Espera-se que isto preserve gradientes úteis para o gerador e reduza a saturação de D(G(z)).
...
RUN_NAME = "g2_discriminator_balancing"
...
for epoch in epoch_bar:
    ...
    for real_images in batch_bar:
        ...
        # No g2 usamos o mesmo alvo suavizado para evitar targets demasiado extremos.
...
# **FID/KID:**
# - O g2 funcionou parcialmente, mas não é melhor que o g1 em FID/qualidade final.
# 
# | Epoch |    FID | KID mean |
# | ----: | -----: | -------: |
# |    50 | 264.99 |   0.1502 |
...
# - Comparando com o g1:
# 
# | Métrica          | g1_more_data | g2_discriminator_balancing |
# | ---------------- | -----------: | -------------------------: |
# | FID final        |   **208.99** |                     244.18 |
# | KID final        |   **0.0887** |                     0.1317 |
# | Melhor FID       |   **208.99** |                     244.18 |
# | Qualidade visual |       melhor |     razoável, mas inferior |
# 
# - Portanto, em métrica quantitativa, o g2 piorou relativamente ao g1.
# 
# **O que melhorou no G2:**
# - O treino ficou mais equilibrado.
# - No g1, o discriminador saturava muito:
# 
#        D(x) ≈ 0.99
#        D(G(z)) ≈ 0.005
#        Loss_G ≈ 6
#        Loss_D ≈ 0
# 
# - No g2, pelo gráfico, o discriminador já não fica tão extremo:
# 
#        D(x) fica mais perto de ~0.85–0.90
#        D(G(z)) não colapsa tão agressivamente no início
#        Loss_G cresce menos do que no g1
#        Loss_D mantém-se mais alto
# 
# - Isto mostra que a alteração fez o que era suposto em termos adversariais: o discriminador ficou menos dominante.
# 
# **O Problema:**
# - Apesar de o treino estar mais equilibrado, a qualidade medida por FID/KID piorou. Isto sugere que o discriminador ficou talvez demasiado fraco/lento, e deixou de dar um sinal suficientemente exigente ao gerador.
# - Ou seja:
# 
#        g1: discriminador demasiado forte, mas melhor FID.
#        g2: treino mais equilibrado, mas pior FID.
# 
# - Isto é um excelente resultado experimental, mesmo sendo “negativo”, porque mostra que o balanceamento simples não foi suficiente.
# 
# **Decisão sobre G2:**
# - O balanceamento reduziu a saturação do discriminador, mas piorou FID/KID. Portanto, não mantemos esta configuração como melhor versão final. Usamos o g1 como base para a próxima melhoria.

# ---
# 
# In g2, we reduced the discriminator learning rate and applied label smoothing to real targets. This reduced discriminator saturation and made the adversarial game less extreme. However, FID worsened from 208.99 in g1 to 244.18, and KID also increased. This suggests that the discriminator became too weak to provide a sufficiently informative training signal. Therefore, g2 was kept as a documented negative result, and the next iteration uses g1 as the stronger base.

# | g2 | discriminator_balancing | Reduce discriminator LR from 1e-4 to 5e-5 and apply real label smoothing 0.9 | FID worsened from g1 208.99 to 244.18; KID worsened from 0.0887 to 0.1317 | Negative/partial result. The discriminator became less saturated, but quantitative image quality decreased. Do not keep as final; use g1 as base for EMA. |
